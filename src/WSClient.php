...
class WSClient implements EventEmitterInterface
{
     ...
     * @return PromiseInterface<WsFrame>
     */
    private function sendReplyAsync(string $reqId, array $body, string $cmd, int $timeout = 0): PromiseInterface
    {
        if ($timeout === 0) {
            $timeout = $this->options->replyAckTimeout;
        }
     ...
     *
     * @param WsFrame $frame 收到的原始 WebSocket 帧
     * @return bool true 表示有消息正在等待 ack
     */
    public function hasPendingReplyAck(WsFrame $frame): bool
    {
        $reqId = $frame->headers->reqId ?? '';
        return $this->connectionManager->hasPendingAck($reqId);
    }
     ...
     * @param array<ReplyMsgItem>|null $msgItem 图文混排项（仅在 finish=true 时有效），用于在结束时附带图片内容
     * @param ReplyFeedback|null $feedback 反馈信息（仅在首次回复时设置）
     * @return PromiseInterface<WsFrame|string> 正常发送时返回回执帧，跳过时返回 'skipped'
     */
    public function replyStreamNonBlocking(
        WsFrame $frame,
        string $streamId,
        string $content,
        bool $finish = false,
